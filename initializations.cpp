...
{
    return (nx + 1) * (ny + 1) + 1 + 6 * (nx - 1) * (ny - 1) + 2 * 4 * (nx - 1) + 2 * 4 * (ny - 1)
           + 6 + 4;
}
...
void fill_gramm(
    msr &matr, size_t nx, size_t ny, int p, int thread, double a, double b, double c, double d)
{
    size_t stride, start;
    start_and_size(p, thread, ny + 1, start, stride);
             ...
             T f)
{
    ...
    if (0 < i && 0 < j && i < nx && j < ny)
        return w
               * (36 * F(i, j)
                  + 20
                        * (F(i + 0.5, j) + F(i + 0.5, j - 0.5) + F(i, j - 0.5) + F(i - 0.5, j)
                           + F(i - 0.5, j + 0.5) + F(i, j + 0.5))
                  + 4
                        * (F(i + 1, j - 0.5) + F(i + 0.5, j + 1) + F(i - 0.5, j - 0.5)
                           + F(i - 1, j + 0.5) + F(i - 0.5, j - 1) + F(i + 0.5, j + 0.5))
                  + 2
                        * (F(i + 1, j) + F(i + 1, j - 1) + F(i, j - 1) + F(i - 1, j)
                           + F(i - 1, j + 1) + F(i, j + 1)));
    if (i == 0 && 0 < j && j < ny)
        return w
               * (18 * F(i, j) + 20 * (F(i + 0.5, j - 0.5) + F(i + 0.5, j))
    ...
    if (0 < i && i < nx && j == 0)
        return w
               * (18 * F(i, j) + 20 * (F(i - 0.5, j + 0.5) + F(i, j + 0.5))
    ...
    if (i == 0 && j == 0)
        return w
               * (6 * F(i, j) + 4 * F(i + 0.5, j + 0.5) + 10 * (F(i + 0.5, j) + F(i, j + 0.5))
                  + (F(i + 1, j) + F(i, j + 1)));
    if (i == nx && j == ny)
        return w
               * (6 * F(i, j) + 4 * F(i - 0.5, j - 0.5) + 10 * (F(i - 0.5, j) + F(i, j - 0.5))
    ...
    if (i == 0 && j == ny)
        return w
               * (12 * F(i, j) + 10 * (F(i + 0.5, j) + F(i, j - 0.5)) + 20 * F(i + 0.5, j - 0.5)
                  + 4 * (F(i + 0.5, j - 1) + F(i + 1, j - 0.5)) + 2 * F(i + 1, j - 1)
                  + (F(i + 1, j) + F(i, j - 1)));
    if (i == nx && j == 0)
        return w
               * (12 * F(i, j) + 10 * (F(i - 0.5, j) + F(i, j + 0.5)) + 20 * F(i - 0.5, j + 0.5)
...
void fill_right_side(size_t nx,
                     size_t ny,
                     double *right,
                     int p,
                     int thread,
